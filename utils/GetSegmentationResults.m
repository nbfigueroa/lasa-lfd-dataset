function [Segmentation Total_feats Clustering Clust_feats my_color_map] = GetSegmentationResults( data, Psi, iis, title_fig, groups )
...
clust_ids = 1:length(groups);
clust_id_init = max(Total_feats);
clust_ids = clust_ids + clust_id_init;

Clust_feats = Total_feats;
for k = 1:length(groups)
    group = groups{k};
    for j=1:length(group)
        Clust_feats(find(Clust_feats == group(j))) = clust_ids(k);
    end
end

Clustering = Segmentation;
for i=1:length(Clustering)
    feat_id = Clustering{i}(:,1);
    for k = 1:length(groups)
        group = groups{k};
        for j=1:length(group)
            feat_id(find(feat_id == group(j))) = clust_ids(k);
        end
    end
    clust = [feat_id Clustering{i}(:,2)];
    Clustering{i,1} = clust;    
end
